...
<li class=""><NuxtLink to="/4.0.0/">4.0.0</NuxtLink></li>
        ...
        <div class="content">
          <h1 class="">Limit Method</h1>
					<p>Limit Method is part of database class builder for limiting query result. It will produce same as <i>SELECT * FROM table WHERE 1 LIMIT [A],[B]</i>.</p>
					<h2>Parameters</h2>
					<p>Update method has 2 required parameters that is <b>offset</b> and <b>count</b>.</p>
					<pre>$this->db->limit(int $offset, int $count): dbObject</pre>
          <h3>$offset</h3>
  ...
  public function latest3ExceptOne(){
    ...
    return $this->db->get();
        ...
        <div class="columns">
          <div class="column">
            <b-button tag="router-link" to="/4.0.0/model/order_by" type="is-link" icon-pack="fa" icon-left="chevron-left" class="is-pulled-left">
              Model::order_by
            </b-button>
          </div>
          <div class="column is-2">&nbsp;</div>
          <div class="column">
            <b-button tag="router-link" to="/4.0.0/model/get" type="is-link" icon-pack="fa" icon-right="chevron-right" class="is-pulled-right">
              Model::get
            </b-button>
          </div>
        </div>
...
<script>
  export default {
    ...
    data() {
      return {
        title: 'Model::limit - Seme Framework v4.0.0'
      }
    },
    head() {
      return {
